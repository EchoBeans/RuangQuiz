...
Public Class FormUbahMateriGuru
    ...
    Public Property MateriID As Integer

    Private Sub FormUbahMateriGuru_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        If MateriID <> 0 Then
            TampilDataLama()
        End If
    ...
    Private Sub TampilDataLama()
        bukaKoneksi()
        Try
            Dim query As String = "SELECT * FROM materi WHERE id=@id"
            cmd = New MySqlCommand(query, conn)
            cmd.Parameters.AddWithValue("@id", MateriID)
            dr = cmd.ExecuteReader()
            If dr.Read() Then
                textBoxJudul.Text = dr("judul").ToString()
                textBoxDeskripsi.Text = dr("deskripsi").ToString()
                textBoxMateri.Text = dr("link").ToString()
            End If
    ...
    Private Sub buttonUbah_Click(sender As Object, e As EventArgs) Handles buttonUbah.Click
        ...
        bukaKoneksi()
        Try
            Dim query As String = "UPDATE materi SET judul=@j, deskripsi=@d, link=@l WHERE id=@id"
            cmd = New MySqlCommand(query, conn)
            cmd.Parameters.AddWithValue("@j", textBoxJudul.Text.Trim())
            cmd.Parameters.AddWithValue("@d", textBoxDeskripsi.Text.Trim())
            cmd.Parameters.AddWithValue("@l", textBoxMateri.Text.Trim())
            cmd.Parameters.AddWithValue("@id", MateriID)
            cmd.ExecuteNonQuery()
            ...
            Me.Hide()
            FormMateriGuru.Show()
            FormMateriGuru.TampilData()

        Catch ex As Exception
            MsgBox("Gagal mengubah data: " & ex.Message, vbCritical)
        Finally
            tutupKoneksi()
        End Try
    End Sub
