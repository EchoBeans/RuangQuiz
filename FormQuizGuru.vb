...
Public Class FormQuizGuru
    ...
    Private Sub FormQuizGuru_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        TampilData()
        AturLabelUser()
    End Sub
    ...
    Private Sub buttonHapus_Click(sender As Object, e As EventArgs) Handles buttonHapus.Click
        ...
        Dim hasil As DialogResult = MessageBox.Show("Yakin ingin menghapus quiz ini?", "Konfirmasi", MessageBoxButtons.YesNo, MessageBoxIcon.Question)

        If hasil = DialogResult.Yes Then
            bukaKoneksi()
            Try
                cmd = New MySqlCommand("DELETE FROM quiz WHERE id=@id", conn)
                cmd.Parameters.AddWithValue("@id", id)
                cmd.ExecuteNonQuery()
                MsgBox("Quiz berhasil dihapus.", vbInformation)
                TampilData()
            Catch ex As Exception
                MsgBox("Gagal menghapus quiz: " & ex.Message, vbCritical)
            Finally
                tutupKoneksi()
            End Try
        End If
    ...
    Private Sub buttonEdit_Click(sender As Object, e As EventArgs) Handles buttonEdit.Click
        ...
        Dim id As Integer = Convert.ToInt32(DataGridView1.CurrentRow.Cells("id").Value)
        Dim formUbah As New FormUbahQuiz()
        formUbah.QuizID = id
        formUbah.Show()
        Me.Hide()
    End Sub
